...
# Usage:
#
#   USERNAME=username USER_PASSWORD=your_password bash <(curl -s https://raw.githubusercontent.com/kurko/devops/master/devops/basic/init.sh)
#
#   - On subsequent runs, you can omit USER_PASSWORD variable because the
#     new user will already have been created.
#   - If not user is specified, uses `whoami`
#
# This script:
#
# - Installs:
...
# - Creates touch /home/$USERNAME/.ssh/authorized_keys
# - Installs Docker and Compose
# - Installs Ruby, adds rbenv to path
# - Installs Node.js and Yarn

# Exits if $USERNAME is either empty or non-existent.
if [ -z "$USERNAME" ]; then
  echo "Please set the USERNAME environment variable so this user can be created."
  exit 1
fi

# Check if the specified user via env var NEW_USER already exists
if ! id -u $USERNAME > /dev/null 2>&1; then
    # If env var USER_PASSWORD is not set, exit
    if [ -z "$USER_PASSWORD" ]; then
      echo "Please set the USER_PASSWORD environment variable so this user can be created."
      exit 1
    fi

    # Create the new user and set a password
...
# Add the '$USERNAME' user to the 'sudo' group
