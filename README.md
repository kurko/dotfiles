...
* **logs you into GitHub** with `gh auth login` (opens a browser), which also
  configures git's credential helper so HTTPS pushes work
...
It opens a browser, stores a token, and sets up git's credential helper, so
HTTPS clones and pushes authenticate without an SSH key.
